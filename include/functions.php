 ...
 * @return string
 */
function wgblocks_block_addCatSelect($cats)
{
    $cat_sql = '(';
    if (\is_array($cats)) {
...
function wgblocksMetaKeywords($content)
{
...
function wgblocksMetaDescription($content)
{
    global $xoopsTpl, $xoTheme;
    $myts = MyTextSanitizer::getInstance();
    $content = $myts->undoHtmlSpecialChars($myts->displayTarea($content));
 ...
 */
function wgblocks_RewriteUrl($module, $array, $type = 'content')
{
    $comment = '';
    ...
    if (isset($array['topic_alias']) && $array['topic_alias']) {
        ...
        $topic_name = wgblocks_Filter(xoops_getModuleOption('static_name', $module));
    }
 ...
 * @return string $url
 */
function wgblocks_Filter($url, $type = '') {
